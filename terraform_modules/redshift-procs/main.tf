...
resource "aws_redshiftdata_statement" "apply_proc" {
  for_each          = var.procedures
  database          = var.database
  workgroup_name    = var.workgroup_name      # for Serverless
  cluster_identifier= var.cluster_identifier  # or Provisioned (use one)
  secret_arn        = var.secret_arn

  sql             = file(each.value.sql_path)
  statement_name  = "apply_${each.key}_${filesha1(each.value.sql_path)}"
  with_event      = true
}
